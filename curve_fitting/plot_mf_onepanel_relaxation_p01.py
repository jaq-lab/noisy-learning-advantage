...
# fig1_relaxation_main notebook (η = 5% solid, 1% dashed — opposite of ETA_STYLES above)
ETA_STYLES_FIG1_MS = {0.05: "-", 0.01: "--"}

# Manuscript Fig.1: linestyles encode MF protocol (η is lighter/darker instead)
METHOD_LS_FIG1_MS = {"hypergraph": "-.", "eigenshadow": "-", "ml": "--"}

# Manuscript Fig.1 device colors: red A, blue B (overridable via device_base_colors)
FIG1_MS_DEVICE_DEFAULT = {"A": "#C62828", "B": "#1565C0"}

FIG1_MS_LW_MAIN = 1.8  # main curves / ML tail / RB connectors only
FIG1_MS_LEG_LW = 1.0  # MF method & η legend Line2D proxies
FIG1_MS_LEG_LW_ALPHA = 2.0  # |α| legend only
FIG1_MS_XLIM = (10.0, 52.0)
FIG1_MS_YMIN = 1e3

# Manuscript Fig.1 dual-|α|: tab10-style green (η-key 0.01) / red (0.05) → n_q/2 / n_q.
...
# η = 5% vs 1%: dark vs light grey; curves blend |α| hue with these greys.
FIG1_MS_ETA_GREY_DARK = "#4a4a4a"
# η=1%: lighter grey in blend → paler curves; slightly darker grey in legend → visible swatch.
FIG1_MS_ETA_GREY_LIGHT_BLEND = "#f5f5f5"
FIG1_MS_ETA_GREY_LIGHT_LEGEND = "#c0c0c0"
FIG1_MS_ALPHA_ETA_GREY_BLEND = 0.22
# η=1%: same opacity as 5%; lighter appearance only via blend toward white on strokes (col_line).
...
# Horizontal refs for manuscript Fig.1 (same as original notebook; differs from TIME_REF_LINES)
FIG1_MS_DAY_NC = 9e10
...
FIG1_MS_YTICKS = [1e0, 1e5, 1e10, 1e15, 1e20, 1e25]
FIG1_MS_YTICKS_ZOOM = [1e5, 1e10, 1e15, 1e20, 1e25]

# Match figures_notebooks/single_column.mplstyle legend.fontsize
...
def _fig1_device_eta_color(hex_color: str, eta: float) -> str:
    """Blend |α| hue with dark vs light grey so η reads as grey dark/light."""
    eta = float(eta)
...
def _fig1_lighten_line_color(hex_color: str, toward_white: float) -> str:
    """Return hex blended toward white; toward_white in [0,1] is fraction of white."""
...
def plot_onepanel(
    ...
    *,
    ...
    if manuscript_fig1 and manuscript_dual_alpha_rows:
        parts = []
        # Draw |α|=n_q/2 (green) first, then |α|=n_q (red) so red sits on top where curves overlap.
        for k in ("nq2", "nq"):
            ...
            if g:
                parts.append((k, g))
        if not parts:
            raise RuntimeError("No rows matched manuscript_dual_alpha_rows filters.")
    else:
    ...
    dev_colors = {**DEVICE_BASE_COLORS, **(device_base_colors or {})}
    if manuscript_fig1:
        dev_colors = {**FIG1_MS_DEVICE_DEFAULT, **dev_colors}

    eta_ls_map = ETA_STYLES_FIG1_MS if manuscript_fig1 else ETA_STYLES
    # Use caller order; manuscript main lines use zorder so η=5% stays above η=1% when paths overlap.
    eta_plot_order = list(etas)
                ...
                for eta in eta_plot_order:
                    ...
                    if np.any(rb):
                        ...
                        if method != "ml":
                            ...
                            # Same color / thickness / style / alpha as main curve (random-guess segment + bridge).
                            ax.plot(
                                x_rb,
                                y_rb,
                                color=col_line,
    ...
    if manuscript_fig1:
        ...
        dev_handles = [
            Line2D([0], [0], color=dev_colors.get(d, "#444444"), linewidth=3.0, label=f"Device {d}")
            for d in devices
        ]
        eta_handles = [
